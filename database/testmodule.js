...
db.serialize(() => {
  ...
  db.run('CREATE TABLE IF NOT EXISTS users (id TEXT PRIMARY KEY, password TEXT)');
  // 예제 사용자 데이터를 삽입합니다. 중복된 id 값이 있으면 무시됩니다.
  db.run("INSERT OR IGNORE INTO users (id, password) VALUES ('user1', 'password123')");
  db.run("INSERT OR IGNORE INTO users (id, password) VALUES ('user2', 'password456')");
});

// 사용자의 아이디와 비밀번호를 확인하는 함수
function checkUserCredentials(id, password, callback) {
  // 데이터베이스에서 id와 password가 일치하는 사용자를 찾습니다.
  db.get('SELECT * FROM adminData WHERE id = ? AND password = ?', [id, password], (err, row) => {
    if (err) {

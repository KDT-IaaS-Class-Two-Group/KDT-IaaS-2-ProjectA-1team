...
이 파일의 경로는
KDT-IaaS-2-ProjectA-1team > frontnext > src > app > lib > 안에 있고
...
또, 이 파일을 나눈다면 db에서 데이터를 읽어오는 부분만 파일로 만들고 id와 password를 입력 받을 수 있는 function 을 만들고, 한 곳에 모아서 읽어온 데이터 id , password 를 이용해서 함수를 작동시키고 싶어

파일을 나눈다면 db에서 데이터를 읽어오는 일만 하는 readDataFromLogin.tsx 와 id와 password 매개변수를 뚫어놓은 function이 있는 loginfunc.tsx와 두 파일을 불러와서 사용하는 logincheck.tsx로 구성되면 깔끔할 것 같아

생성되는 파일들의 위치는 KDT-IaaS-2-ProjectA-1team > frontnext > src > app > lib > 안에 생성되면 좋겠어